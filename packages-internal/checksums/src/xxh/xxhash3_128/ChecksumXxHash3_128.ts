...
export class ChecksumXxHash3_128 extends XxHash3Base implements Checksum {
  ...
  private concat(hiSlot: index, loSlot: index): Uint8Array {
    const result = new Uint8Array(16);
    result.set(this.mem.toBytes(hiSlot), 0);
    result.set(this.mem.toBytes(loSlot), 8);
    return result;
  }
  ...
  private mixTwoChunks(data: Uint8Array, off1: index, off2: index, secretOff: index): void {
    const { mem } = this;
    const [d100, d101] = readU64LE(data, off1);
    const [d110, d111] = readU64LE(data, off1 + 8);
    const [d200, d201] = readU64LE(data, off2);
    const [d210, d211] = readU64LE(data, off2 + 8);
    const ms1 = this.mixStep(data, off1, secretOff);
    mem.add(T0, mem.hi(ms1), mem.lo(ms1));
    const ms2 = this.mixStep(data, off2, secretOff + 16);
