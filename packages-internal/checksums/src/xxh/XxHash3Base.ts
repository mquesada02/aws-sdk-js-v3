...
export abstract class XxHash3Base {
  ...
  protected accumulateStripe(input: Uint8Array, stripeOff: index, secretOff: index): void {
    const { mem } = this;
    const secret = this.secret;
    for (let i = 0; i < 8; i++) {
      const [d0, d1] = readU64LE(input, stripeOff + i * 8);
      const [s0, s1] = readU64LE(secret, secretOff + i * 8);
      const val0 = (d0 ^ s0) >>> 0;
      const val1 = (d1 ^ s1) >>> 0;
      mem.add(ACC + (i ^ 1), d0, d1);
      mem.write(T1, 0, val1).write(T2, 0, val0);
      mem.multiply128(T1, T2, T4, T3);
      mem.add(ACC + i, mem.hi(T3), mem.lo(T3));
    }
  }
  ...
  protected roundScramble(): void {
    ...
    const secret = this.secret;
    for (let i = 0; i < 8; i++) {
      const [s0, s1] = readU64LE(secret, SECRET_LENGTH - 64 + i * 8);
  ...
  protected finalMerge(init0: u64_l, init1: u64_r, secretOff: index): Uint8Array {
    const { mem } = this;
    const secret = this.secret;
    mem.write(T0, init0, init1);
    for (let i = 0; i < 4; i++) {
      const [s00, s01] = readU64LE(secret, secretOff + i * 16);
      const [s10, s11] = readU64LE(secret, secretOff + i * 16 + 8);
  ...
  protected mixStep(data: Uint8Array, dataOff: index, secretOff: index): index {
    const { mem } = this;
    const secret = this.secret;
    const [d00, d01] = readU64LE(data, dataOff);
    const [d10, d11] = readU64LE(data, dataOff + 8);
    const [s00, s01] = readU64LE(secret, secretOff);
    const [s10, s11] = readU64LE(secret, secretOff + 8);
  ...
  /** avalanche(x): x ^= x >> 37; x *= PRIME_MX1; x ^= x >> 32; Result in `slot`. */
  protected avalanche(slot: index, v0: u64_l, v1: u64_r): void {
    const { mem } = this;
    mem.write(slot, v0, v1);
    mem.copy(slot, T3).right(T3, 37);
    mem.xor(slot, mem.hi(T3), mem.lo(T3));
    mem.multiply(slot, PRIME_MX1_0, PRIME_MX1_1);
    mem.copy(slot, T3).right(T3, 32);
    mem.xor(slot, mem.hi(T3), mem.lo(T3));
  }

  /** avalanche_XXH64(x): x ^= x>>33; x *= P2; x ^= x>>29; x *= P3; x ^= x>>32; Result in `slot`. */
  protected avalanche64(slot: index, v0: u64_l, v1: u64_r): void {
    const { mem } = this;
    mem.write(slot, v0, v1);
    mem.copy(slot, T3).right(T3, 33);
    mem.xor(slot, mem.hi(T3), mem.lo(T3));
    mem.multiply(slot, PRIME64_2_0, PRIME64_2_1);
    mem.copy(slot, T3).right(T3, 29);
    mem.xor(slot, mem.hi(T3), mem.lo(T3));
    mem.multiply(slot, PRIME64_3_0, PRIME64_3_1);
    mem.copy(slot, T3).right(T3, 32);
    mem.xor(slot, mem.hi(T3), mem.lo(T3));
  }
